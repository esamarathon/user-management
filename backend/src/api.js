...
import {
  sendDiscordSubmission, sendDiscordSubmissionUpdate, sendDiscordSubmissionDeletion, sendDiscordSubmissionDecision, sendDiscordVolunteerDecision
...
export async function getSubmissions(req, res) {
  ...
  if (user && hasPermission(user, req.query.event, runDecisionPermission)) {
    ...
    runs = await models.Submission.find({ event: req.query.event, status: { $in: ['saved', 'accepted', 'rejected'] } }, 'status createdAt event user game twitchGame leaderboards category platform estimate runType runners video comment decisions incentives')
